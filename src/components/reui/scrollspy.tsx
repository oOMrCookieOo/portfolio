...
type ScrollspyProps = {
  ...
  throttleTime?: number
  /**
   * Local addition: when false, anchors keep their native behaviour instead of
   * being hijacked. Needed when a smooth-scroll library owns the scroll position
   * (Lenis here): this component's window.scrollTo is overridden on the library's
   * next frame, so the page moves a few pixels and stops.
   */
...
export function Scrollspy({
  ...
  const handleScroll = useCallback(() => {
    ...
    // Local fix: while the page is still short (lazy sections not mounted yet)
    // this test is true at scrollTop 0 and wrongly forces the last anchor active.
  ...
  useEffect(() => {
    ...
    // LOCAL PATCH: handleScroll reads offsetTop for every anchor, so running it
    // per scroll event laid the page out dozens of times a second under Lenis.
    // Coalesced to one run per frame, and the listener is passive.
    let queued = 0
    ...
    const initialTimeout = setTimeout(() => {
      if (handleClicks) scrollToHashSection()
      handleScroll()
    }, 100)
