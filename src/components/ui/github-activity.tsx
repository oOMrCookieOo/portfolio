...
function useGitHubUser(login?: string) {
  ...
  // LOCAL PATCH: upstream swallowed failures, so a rate-limited or offline fetch
  // left the placeholder in place while the heading claimed "0 contributions".
...
const GitHubActivity = ({
  ...
  // LOCAL PATCH: never assert a count that has not arrived.
  const heading =
    fetched?.state === "loading"
      ? "Loading contributions"
      : fetched?.state === "failed"
